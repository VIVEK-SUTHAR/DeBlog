export default function BlogCard({ blogdata }) {
  const filename = blogdata.coverImage.split("/")[3];
  const filehash = blogdata.coverImage.split("/")[2].split(".")[0];
  ...
  return (
    <div className="max-h-full min-h-[368px] mb-8 w-96 shadow-2xl flex flex-col rounded-xl p-4 pt-6 overflow-hidden border-2  hover:bg-[hsla(0,0%,100%,.1)] hover:transition-all hover:delay-7">
      <div className="w-full">
        <img src={img_url} alt="banner" className="h-36 w-full rounded-xl" />
      </div>
      <div className="h-full pt-4 pr-8 text-black flex-1 flex flex-col justify-start">
        ...
        <h4 className="text-xl text-[#4b4b4b]">
          By{" "}
          {blogdata.blogOwner.slice(0, 4) +
            "..." +
            blogdata.blogOwner.slice(
              blogdata.blogOwner.length - 4,
              blogdata.blogOwner.length
            )}
        </h4>
      </div>
    </div>
